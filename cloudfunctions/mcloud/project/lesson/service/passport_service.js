...
class PassportService extends BaseProjectService {
	...
	async getMyDetail(userId) {
		...
		let fields = 'USER_COURSE_TOTAL_CNT,USER_COURSE_USED_CNT,USER_MOBILE,USER_NAME,USER_FORMS,USER_OBJ,USER_STATUS,USER_CHECK_REASON'
		let user = await UserModel.getOne(where, fields);

		if (user) {
			let where = {
				JOIN_USER_ID: userId,
				JOIN_DAY: ['>=', timeUtil.time('Y-M-D')]
			}
			// 最新排课
			let myCnt = await JoinModel.count(where);
			user.myCnt = myCnt;
